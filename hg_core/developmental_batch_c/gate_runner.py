"""Shared proof gate runner for Batch D4-C."""
...
SLICE_TEST_TARGETS: dict[str, list[str]] = {
    "sab": ["tests/sab", "tests/developmental_batch_c/test_all_slices.py::test_each_slice_green"],
    "iab": ["tests/iab", "tests/developmental_batch_c/test_all_slices.py::test_each_slice_green"],
    "trl": ["tests/trl", "tests/developmental_batch_c/test_all_slices.py::test_each_slice_green"],
    "all": [
        "tests/sab",
        "tests/iab",
        "tests/trl",
        "tests/developmental_batch_c",
    ],
}
...
def run_developmental_gate(workspace: Path, *, slice: str, gate_id: str, proof_subpath: str) -> int:
    ...
    checks = run_developmental_batch_c_checks(workspace, slice=slice)
    ...
    gate_result: dict[str, Any] = {
        ...
        "slice": slice,
        "ok": gate_ok,
        "slices": list(D4_C_SLICES) if slice == "all" else [slice],
        "verdicts": [
            {
