"""BRS cluster import fences — no OEA/TER/GPP/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[1] / "brs_cluster",
    Path(__file__).resolve().parents[2] / "hg_runtime" / "bus_rate_supervisor",
)


def check_brs_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
__all__ = ["advisory_only_marker", "check_brs_import_fences"]
