"""DBB cluster import fences — no OEA/TER/GPP/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[1] / "dbb_cluster",
    Path(__file__).resolve().parents[2] / "hg_runtime" / "data_blob_bus",
)


def check_dbb_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
__all__ = ["advisory_only_marker", "check_dbb_import_fences"]
