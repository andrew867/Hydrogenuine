"""Shared proof gate runner for DBB Data/Blob Bus."""
...
def run_dbb_feature_checks() -> dict[str, object]:
    ...
    from hg_core.dbb_cluster.no_authority import check_dbb_import_fences
    ...
    checks.append({"check_id": "static_fixtures_only_default", "ok": dbb_static_fixtures_only() and dbb_refuse_live_model_invocation(), "detail": "fixture/static slice enforced"})
    checks.append({"check_id": "refuse_authority_conversion_default", "ok": dbb_refuse_authority_conversion(), "detail": "authority conversion refusal enabled"})
    rtc_ok, rtc_failures = validate_dbb_rtc_event_design(planned_dbb_event_refs())
    checks.append({"check_id": "rtc_event_design", "ok": rtc_ok, "detail": rtc_failures if not rtc_ok else len(planned_dbb_event_refs())})
    fences_ok, fence_detail = check_dbb_import_fences()
    checks.append({"check_id": "import_fences", "ok": fences_ok, "detail": fence_detail if not fences_ok else "clean"})
    analysis = analyze_dbb_fixtures(observed_at=FIXTURE_CLOCK)
    ...
    bundles = load_dbb_fixtures()
    valid_bundle = next(b for b in bundles if b["bundle_id"] == "dbb-valid-blob")
    valid_result = process_dbb_bundle(valid_bundle, observed_at=FIXTURE_CLOCK)
    ...
    fail_bundle = next(b for b in bundles if b["bundle_id"] == "dbb-stale-input")
    fail_result = process_dbb_bundle(fail_bundle, observed_at=FIXTURE_CLOCK)
    ...
    receipt = valid_result.get("dbb_receipt")
    ...
    for adv_id in ("token-grant", "context-grant", "execution-admission"):
        adv_bundle = next(b for b in bundles if b["bundle_id"] == f"dbb-{adv_id}")
        adv_result = process_dbb_bundle(adv_bundle, observed_at=FIXTURE_CLOCK)
        checks.append(
            {
                "check_id": f"adversarial_dbb-{adv_id}",
...
def run_dbb_gate(workspace: Path, *, gate_id: str = "dbb_data_blob_bus_gate_v1") -> int:
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    proof_dir = workspace / "docs" / "proofs" / "autonomic_runtime_multibus" / "DBB" / ts
    artifacts_dir = proof_dir / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    command_log = proof_dir / "command_log.jsonl"
    command_log.write_text("", encoding="utf-8")
    feature_checks = run_dbb_feature_checks()
    (artifacts_dir / "dbb_feature_checks.json").write_text(json.dumps(feature_checks, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    test_targets = ["tests/dbb"]
    t0 = time.monotonic()
    ...
    gate_result: dict[str, Any] = {"gate": gate_id, "ok": gate_ok, "verdicts": [{"check": "dbb_feature_checks", "verdict": "pass" if feature_checks["ok"] else "fail", "ok": feature_checks["ok"]}, {"check": "focused_unit_tests", "verdict": "pass" if test_cmd.returncode == 0 else "fail", "ok": test_cmd.returncode == 0}], "critical_failures": feature_checks.get("critical_failures", [])}
    (proof_dir / "gate_result.json").write_text(json.dumps(gate_result, indent=2), encoding="utf-8")
    (proof_dir / "manifest.json").write_text(json.dumps({"schema": "ct_proof_bundle_v1", "pack": "DBB", "gate": gate_id, "timestamp": ts, "head": git_head(workspace), "path_id": "autonomic_runtime_multibus", "file_hashes": {"gate_result.json": sha256_file(proof_dir / "gate_result.json"), "artifacts/dbb_feature_checks.json": sha256_file(artifacts_dir / "dbb_feature_checks.json"), "command_log.jsonl": sha256_file(command_log)}}, indent=2), encoding="utf-8")
    if not gate_ok:
        print("DBB gate FAILED", file=sys.stderr)
        return 1
    print(f"DBB gate GREEN — proof bundle: {proof_dir}")
    return 0


__all__ = ["run_dbb_feature_checks", "run_dbb_gate"]
