"""Shared proof gate runner for RDB Reproduction/Delegation Bus."""
...
def run_rdb_feature_checks() -> dict[str, object]:
    ...
    from hg_core.rdb_cluster.no_authority import check_rdb_import_fences
    ...
    checks.append({"check_id": "static_fixtures_only_default", "ok": rdb_static_fixtures_only() and rdb_refuse_live_model_invocation(), "detail": "fixture/static slice enforced"})
    checks.append({"check_id": "refuse_authority_conversion_default", "ok": rdb_refuse_authority_conversion(), "detail": "authority conversion refusal enabled"})
    rtc_ok, rtc_failures = validate_rdb_rtc_event_design(planned_rdb_event_refs())
    checks.append({"check_id": "rtc_event_design", "ok": rtc_ok, "detail": rtc_failures if not rtc_ok else len(planned_rdb_event_refs())})
    fences_ok, fence_detail = check_rdb_import_fences()
    checks.append({"check_id": "import_fences", "ok": fences_ok, "detail": fence_detail if not fences_ok else "clean"})
    analysis = analyze_rdb_fixtures(observed_at=FIXTURE_CLOCK)
    ...
    valid_bundle = next(b for b in bundles if b["bundle_id"] == "rdb-valid-delegation")
    valid_result = process_rdb_bundle(valid_bundle, observed_at=FIXTURE_CLOCK)
    ...
    fail_bundle = next(b for b in bundles if b["bundle_id"] == "rdb-stale-input")
    fail_result = process_rdb_bundle(fail_bundle, observed_at=FIXTURE_CLOCK)
    ...
    receipt = valid_result.get("rdb_receipt")
    ...
    for adv_id in ("token-grant", "context-grant", "execution-admission"):
        adv_bundle = next(b for b in bundles if b["bundle_id"] == f"rdb-{adv_id}")
        adv_result = process_rdb_bundle(adv_bundle, observed_at=FIXTURE_CLOCK)
        checks.append(
            {
                "check_id": f"adversarial_rdb-{adv_id}",
...
def run_rdb_gate(workspace: Path, *, gate_id: str = "rdb_reproduction_delegation_bus_gate_v1") -> int:
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    proof_dir = workspace / "docs" / "proofs" / "autonomic_runtime_multibus" / "RDB" / ts
    artifacts_dir = proof_dir / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    command_log = proof_dir / "command_log.jsonl"
    command_log.write_text("", encoding="utf-8")
    feature_checks = run_rdb_feature_checks()
    (artifacts_dir / "rdb_feature_checks.json").write_text(json.dumps(feature_checks, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    test_targets = ["tests/rdb"]
    t0 = time.monotonic()
    ...
    gate_result: dict[str, Any] = {"gate": gate_id, "ok": gate_ok, "verdicts": [{"check": "rdb_feature_checks", "verdict": "pass" if feature_checks["ok"] else "fail", "ok": feature_checks["ok"]}, {"check": "focused_unit_tests", "verdict": "pass" if test_cmd.returncode == 0 else "fail", "ok": test_cmd.returncode == 0}], "critical_failures": feature_checks.get("critical_failures", [])}
    (proof_dir / "gate_result.json").write_text(json.dumps(gate_result, indent=2), encoding="utf-8")
    (proof_dir / "manifest.json").write_text(json.dumps({"schema": "ct_proof_bundle_v1", "pack": "RDB", "gate": gate_id, "timestamp": ts, "head": git_head(workspace), "path_id": "autonomic_runtime_multibus", "file_hashes": {"gate_result.json": sha256_file(proof_dir / "gate_result.json"), "artifacts/rdb_feature_checks.json": sha256_file(artifacts_dir / "rdb_feature_checks.json"), "command_log.jsonl": sha256_file(command_log)}}, indent=2), encoding="utf-8")
    if not gate_ok:
        print("RDB gate FAILED", file=sys.stderr)
        return 1
    print(f"RDB gate GREEN — proof bundle: {proof_dir}")
    return 0


__all__ = ["run_rdb_feature_checks", "run_rdb_gate"]
