"""Shared proof gate runner for MEM-LIVE governed live memory mutation."""
...
def run_mem_feature_checks() -> dict[str, object]:
    ...
    from hg_core.mem_live.no_authority import check_mem_import_fences
    ...
    checks.append(
        {
            ...
            "ok": mem_fake_sink_only() and mem_refuse_durable_writes(),
    ...
    checks.append(
        {
            ...
            "ok": mem_refuse_authority_conversion(),
            "detail": "authority conversion refusal enabled",
        }
    )
    fences_ok, fence_detail = check_mem_import_fences()
    ...
    checks.append(
        {
            ...
            and analysis.get("no_durable_writes") is True
    ...
    valid_bundle = next(b for b in bundles if b["bundle_id"] == "mem-valid-write")
    valid_result = process_mem_bundle(valid_bundle, observed_at=FIXTURE_CLOCK)
    checks.append(
        {
            "check_id": "valid_write_recorded",
            "ok": valid_result.get("status") == "recorded"
            and valid_result.get("permission_granted") is False
            and valid_result.get("durable_write_performed") is False,
            "detail": valid_result.get("reason_code"),
        }
    )

    stale_bundle = next(b for b in bundles if b["bundle_id"] == "mem-stale-approval")
    stale_result = process_mem_bundle(stale_bundle, observed_at=FIXTURE_CLOCK)
    checks.append(
        {
    ...
    missing_iam = next(b for b in bundles if b["bundle_id"] == "mem-missing-iam")
    missing_iam_result = process_mem_bundle(missing_iam, observed_at=FIXTURE_CLOCK)
    checks.append(
        {
    ...
    missing_tim = next(b for b in bundles if b["bundle_id"] == "mem-missing-tim")
    missing_tim_result = process_mem_bundle(missing_tim, observed_at=FIXTURE_CLOCK)
    checks.append(
        {
    ...
    missing_gpp = next(b for b in bundles if b["bundle_id"] == "mem-missing-gpp")
    missing_gpp_result = process_mem_bundle(missing_gpp, observed_at=FIXTURE_CLOCK)
    checks.append(
        {
    ...
    missing_ueak = next(b for b in bundles if b["bundle_id"] == "mem-missing-ueak")
    missing_ueak_result = process_mem_bundle(missing_ueak, observed_at=FIXTURE_CLOCK)
    checks.append(
        {
    ...
    rollback_bundle = next(b for b in bundles if b["bundle_id"] == "mem-valid-rollback")
    rollback_result = process_mem_bundle(rollback_bundle, observed_at=FIXTURE_CLOCK)
    ...
    restore_bundle = next(b for b in bundles if b["bundle_id"] == "mem-valid-restore")
    restore_result = process_mem_bundle(restore_bundle, observed_at=FIXTURE_CLOCK)
    checks.append(
        {
            "check_id": "restore_record_present",
            "ok": isinstance(restore_result.get("restore_result"), dict),
            "detail": restore_result.get("reason_code"),
        }
    )
    ...
    checks.append(
        {
            "check_id": "mutation_adapter_fixture",
            "ok": adapter.get("durable_write_performed") is False and adapter.get("permission_granted") is False,
            "detail": adapter.get("reason_code"),
        }
    )

    for bundle_id, expected_status in (
        ("mem-authority-conversion", "contained"),
        ("mem-out-of-scope-live", "contained"),
    ):
    ...
    secret_bundle = next(b for b in bundles if b["bundle_id"] == "mem-secret-leak")
    secret_result = process_mem_bundle(secret_bundle, observed_at=FIXTURE_CLOCK)
    checks.append(
        {
            "check_id": "adversarial_mem-secret-leak",
...
def run_mem_gate(workspace: Path, *, gate_id: str = "live_memory_mutation_v1") -> int:
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    proof_dir = workspace / "docs" / "proofs" / "live_memory_mutation" / "MEM-LIVE" / ts
    artifacts_dir = proof_dir / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    command_log = proof_dir / "command_log.jsonl"
    command_log.write_text("", encoding="utf-8")

    feature_checks = run_mem_feature_checks()
    (artifacts_dir / "mem_feature_checks.json").write_text(
        json.dumps(feature_checks, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )

    test_targets = ["tests/mem_live"]
    t0 = time.monotonic()
        ...
        json.dumps(
            {
                ...
                "path_id": "live_memory_mutation/MEM-LIVE",
                "file_hashes": {
                    "gate_result.json": sha256_file(proof_dir / "gate_result.json"),
                    "artifacts/mem_feature_checks.json": sha256_file(artifacts_dir / "mem_feature_checks.json"),
    ...
    status_lines = [
        f"# MEM-LIVE Governed Live Memory Mutation — {ts}",
        "",
...
__all__ = ["run_mem_feature_checks", "run_mem_gate"]
