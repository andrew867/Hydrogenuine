"""DRB cluster import fences — no OEA/TER/GPP/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[1] / "drb_cluster",
    Path(__file__).resolve().parents[2] / "hg_runtime" / "dream_reflection_boundary",
)


def check_drb_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
__all__ = ["advisory_only_marker", "check_drb_import_fences"]
