"""NRV cluster import fences — no OEA/TER/GPP/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[1] / "nrv_cluster",
    Path(__file__).resolve().parents[2] / "hg_runtime" / "nervous_routing_layer",
)


def check_nrv_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
__all__ = ["advisory_only_marker", "check_nrv_import_fences"]
