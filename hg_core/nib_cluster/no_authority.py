"""NIB cluster import fences — no OEA/TER/GPP/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[1] / "nib_cluster",
    Path(__file__).resolve().parents[2] / "hg_runtime" / "nutrient_intake_boundary",
)


def check_nib_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
__all__ = ["advisory_only_marker", "check_nib_import_fences"]
