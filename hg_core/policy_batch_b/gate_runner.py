"""Shared proof gate runner for Batch P1-B."""
...
from hg_core.policy_batch_b.checks import P1_B_SLICES, run_policy_batch_b_checks
from hg_core.proof.command_log import record_command

SLICE_TEST_TARGETS: dict[str, list[str]] = {
    "fce": ["tests/fce", "tests/policy_batch_b/test_all_slices.py::test_each_slice_green"],
    "vsp": ["tests/vsp", "tests/policy_batch_b/test_all_slices.py::test_each_slice_green"],
    "cdo": ["tests/cdo", "tests/policy_batch_b/test_all_slices.py::test_each_slice_green"],
    "crt": ["tests/crt", "tests/policy_batch_b/test_all_slices.py::test_each_slice_green"],
    "all": ["tests/fce", "tests/vsp", "tests/cdo", "tests/crt", "tests/policy_batch_b"],
}
...
def run_policy_gate(workspace: Path, *, slice: str, gate_id: str, proof_subpath: str) -> int:
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    proof_dir = workspace / "docs" / "proofs" / "policy_safety" / proof_subpath / ts
    artifacts_dir = proof_dir / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    command_log = proof_dir / "command_log.jsonl"
    command_log.write_text("", encoding="utf-8")

    checks = run_policy_batch_b_checks(workspace, slice=slice)
    (artifacts_dir / "policy_batch_checks.json").write_text(
    ...
    gate_result: dict[str, Any] = {
        ...
        "slice": slice,
        "ok": gate_ok,
        "slices": list(P1_B_SLICES) if slice == "all" else [slice],
        "verdicts": [
            {
                "check": "policy_batch_checks",
        ...
        json.dumps(
            {
                ...
                "path_id": f"policy_safety/{proof_subpath}",
                "file_hashes": {
                    "gate_result.json": sha256_file(proof_dir / "gate_result.json"),
                    "artifacts/policy_batch_checks.json": sha256_file(artifacts_dir / "policy_batch_checks.json"),
...
__all__ = ["SLICE_TEST_TARGETS", "run_policy_gate"]
