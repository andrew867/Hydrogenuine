"""Shared DSE tranche gate runner factory."""
...
from hg_core.dse.proof_bundle import emit_proof_bundle, new_proof_dir
...
def run_tranche_gate(
    ...
    *,
    ...
    test_cmd = subprocess.run(
        [sys.executable, "-m", "pytest", test_dir, "-q", "--timeout=120"],
        cwd=workspace,
        capture_output=True,
        text=True,
        check=False,
    ...
    gate_ok = feature_checks.get("ok") and test_cmd.returncode == 0
    emit_proof_bundle(
        proof_dir,
        pack=pack,
        gate=gate_id,
        head=git_head(workspace),
        artifacts={f"{pack.lower()}_checks.json": feature_checks},
