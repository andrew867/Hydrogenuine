"""Shared proof gate runner for Batch S5-B."""
...
SLICE_TEST_TARGETS: dict[str, list[str]] = {
    "sml": ["tests/sml", "tests/signaling_batch_b/test_all_slices.py::test_each_slice_green"],
    "kar": ["tests/kar", "tests/signaling_batch_b/test_all_slices.py::test_each_slice_green"],
    "obl": ["tests/obl", "tests/signaling_batch_b/test_all_slices.py::test_each_slice_green"],
    "all": [
        "tests/sml",
        "tests/kar",
        "tests/obl",
        "tests/signaling_batch_b",
    ],
}
...
def run_signaling_b_gate(workspace: Path, *, slice: str, gate_id: str, proof_subpath: str) -> int:
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    proof_dir = workspace / "docs" / "proofs" / "signaling" / proof_subpath / ts
    artifacts_dir = proof_dir / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    command_log = proof_dir / "command_log.jsonl"
    command_log.write_text("", encoding="utf-8")

    checks = run_signaling_batch_b_checks(workspace, slice=slice)
    (artifacts_dir / "signaling_batch_checks.json").write_text(
    ...
    gate_result: dict[str, Any] = {
        ...
        "slice": slice,
        "ok": gate_ok,
        "slices": list(S5_B_SLICES) if slice == "all" else [slice],
        "verdicts": [
            {
                "check": "signaling_batch_checks",
        ...
        json.dumps(
            {
                ...
                "path_id": f"signaling/{proof_subpath}",
                "file_hashes": {
                    "gate_result.json": sha256_file(proof_dir / "gate_result.json"),
                    "artifacts/signaling_batch_checks.json": sha256_file(
                        artifacts_dir / "signaling_batch_checks.json"
                    ),
...
__all__ = ["SLICE_TEST_TARGETS", "run_signaling_b_gate"]
