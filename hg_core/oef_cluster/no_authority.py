"""OEF cluster import fences — no OEA/TER/GPP/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[1] / "oef_cluster",
    Path(__file__).resolve().parents[2] / "hg_runtime" / "organ_edge_filter",
)


def check_oef_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
__all__ = ["advisory_only_marker", "check_oef_import_fences"]
