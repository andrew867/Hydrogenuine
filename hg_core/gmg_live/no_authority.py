"""GMG-LIVE import fences — no OEA/TER/GPP mint/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[2] / "hg_runtime" / "grant_authority_live",
)


def check_gmg_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
def advisory_only_marker() -> dict[str, bool]:
    return {
        ...
        "grant_is_advisory_only": True,
        "live_grant_performed": False,
        "live_action_performed": False,
    }


__all__ = ["advisory_only_marker", "check_gmg_import_fences"]
