"""TLB cluster import fences — no OEA/TER/GPP/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[1] / "tlb_cluster",
    Path(__file__).resolve().parents[2] / "hg_runtime" / "tool_lifecycle_boundary",
)


def check_tlb_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
__all__ = ["advisory_only_marker", "check_tlb_import_fences"]
