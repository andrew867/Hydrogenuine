"""Shared proof gate runner for Batch L3-B."""
...
SLICE_TEST_TARGETS: dict[str, list[str]] = {
    "mor": ["tests/mortality_memory_offering", "tests/lifecycle_batch_b/test_all_slices.py::test_each_slice_green"],
    "cnt": ["tests/continuity_boundary", "tests/lifecycle_batch_b/test_all_slices.py::test_each_slice_green"],
    "all": [
        "tests/mortality_memory_offering",
        "tests/continuity_boundary",
        "tests/lifecycle_batch_b",
    ],
}
...
def run_lifecycle_gate(workspace: Path, *, slice: str, gate_id: str, proof_subpath: str) -> int:
    ...
    checks = run_lifecycle_batch_b_checks(workspace, slice=slice)
    ...
    gate_result: dict[str, Any] = {
        ...
        "slice": slice,
        "ok": gate_ok,
        "slices": list(L3_B_SLICES) if slice == "all" else [slice],
        "verdicts": [
            {
