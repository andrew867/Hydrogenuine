"""MBS cluster import fences — no OEA/TER/GPP/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[1] / "mbs_cluster",
    Path(__file__).resolve().parents[2] / "hg_runtime" / "multi_bus_substrate",
)


def check_mbs_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
__all__ = ["advisory_only_marker", "check_mbs_import_fences"]
