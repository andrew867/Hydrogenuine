"""SEN-LIVE import fences — no OEA/TER/GPP mint/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[2] / "hg_runtime" / "live_sensor_ingestion",
)


def check_sen_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
def advisory_only_marker() -> dict[str, bool]:
    return {
        ...
        "sensor_ingestion_is_advisory_only": True,
        "live_sensor_connection": False,
        "live_action_performed": False,
    }


__all__ = ["advisory_only_marker", "check_sen_import_fences"]
