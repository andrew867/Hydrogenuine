"""Shared proof gate runner for Batch R2-C."""
...
SLICE_TEST_TARGETS: dict[str, list[str]] = {
    "pro": ["tests/proprioceptive_body_model", "tests/runtime_context_batch_c/test_all_slices.py::test_each_slice_green"],
    "all": [
        "tests/proprioceptive_body_model",
        "tests/runtime_context_batch_c",
    ],
}
...
def run_runtime_context_gate(workspace: Path, *, slice: str, gate_id: str, proof_subpath: str) -> int:
    ...
    checks = run_runtime_context_batch_c_checks(workspace, slice=slice)
    ...
    gate_result: dict[str, Any] = {
        ...
        "slice": slice,
        "ok": gate_ok,
        "slices": list(R2_C_SLICES) if slice == "all" else [slice],
        "verdicts": [
            {
    ...
    status_lines = [
        f"# R2-C PRO Backburner — {slice} — {ts}",
        "",
