"""Shared proof gate runner for Batch D4-B."""
...
from hg_core.developmental_batch_b.checks import D4_B_SLICES, run_developmental_batch_b_checks
from hg_core.proof.command_log import record_command

SLICE_TEST_TARGETS: dict[str, list[str]] = {
    "rgl": ["tests/rgl", "tests/developmental_batch_b/test_all_slices.py::test_each_slice_green"],
    "scl": ["tests/scl", "tests/developmental_batch_b/test_all_slices.py::test_each_slice_green"],
    "iil": ["tests/iil", "tests/developmental_batch_b/test_all_slices.py::test_each_slice_green"],
    "all": [
        "tests/rgl",
        "tests/scl",
        "tests/iil",
        "tests/developmental_batch_b",
    ],
}
...
def run_developmental_gate(workspace: Path, *, slice: str, gate_id: str, proof_subpath: str) -> int:
    ...
    checks = run_developmental_batch_b_checks(workspace, slice=slice)
    ...
    gate_result: dict[str, Any] = {
        ...
        "slice": slice,
        "ok": gate_ok,
        "slices": list(D4_B_SLICES) if slice == "all" else [slice],
        "verdicts": [
            {
