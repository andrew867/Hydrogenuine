"""IMS cluster import fences — no OEA/TER/GPP/UEAK/SRP apply."""
...
_PACKAGE_ROOTS = (
    Path(__file__).resolve().parents[1] / "ims_cluster",
    Path(__file__).resolve().parents[2] / "hg_runtime" / "inference_model_scheduler",
)


def check_ims_import_fences() -> tuple[bool, list[str]]:
    failures: list[str] = []
...
__all__ = ["advisory_only_marker", "check_ims_import_fences"]
