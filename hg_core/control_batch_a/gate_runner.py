"""Shared proof gate runner for Batch C6-A."""
...
SLICE_TEST_TARGETS: dict[str, list[str]] = {
    "rsc": ["tests/rsc", "tests/control_batch_a/test_all_slices.py::test_each_slice_green"],
    "pab": ["tests/pab", "tests/control_batch_a/test_all_slices.py::test_each_slice_green"],
    "mis": ["tests/mis", "tests/control_batch_a/test_all_slices.py::test_each_slice_green"],
    "gcb": ["tests/gcb", "tests/control_batch_a/test_all_slices.py::test_each_slice_green"],
    "trb": ["tests/trb", "tests/control_batch_a/test_all_slices.py::test_each_slice_green"],
    "rpb": ["tests/rpb", "tests/control_batch_a/test_all_slices.py::test_each_slice_green"],
    "all": [
        "tests/rsc",
        "tests/pab",
        "tests/mis",
...
def run_control_a_gate(workspace: Path, *, slice: str, gate_id: str, proof_subpath: str) -> int:
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    proof_dir = workspace / "docs" / "proofs" / "control" / proof_subpath / ts
    artifacts_dir = proof_dir / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    command_log = proof_dir / "command_log.jsonl"
    command_log.write_text("", encoding="utf-8")

    checks = run_control_batch_a_checks(workspace, slice=slice)
    (artifacts_dir / "control_batch_checks.json").write_text(
    ...
    gate_result: dict[str, Any] = {
        ...
        "slice": slice,
        "ok": gate_ok,
        "slices": list(C6_A_SLICES) if slice == "all" else [slice],
        "verdicts": [
            {
                "check": "control_batch_checks",
        ...
        json.dumps(
            {
                ...
                "path_id": f"control/{proof_subpath}",
                "file_hashes": {
                    "gate_result.json": sha256_file(proof_dir / "gate_result.json"),
                    "artifacts/control_batch_checks.json": sha256_file(
                        artifacts_dir / "control_batch_checks.json"
                    ),
...
__all__ = ["SLICE_TEST_TARGETS", "run_control_a_gate"]
