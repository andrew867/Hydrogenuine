"""Recognition consent FastAPI dependencies (G15)."""
...
def require_consent_surface_enabled() -> None:
    if not is_consent_surface_enabled():
        raise HTTPException(status_code=503, detail="consent_surface_disabled")
...
def require_recognition_consent(
    ...
    """Fail-closed recognition consent guard for operator API routes (CS-1, CS-3)."""
    ...
    if have < need:
        _ledger().deny_request(subject_id=sid, reason="consent_required", source="operator_api")
