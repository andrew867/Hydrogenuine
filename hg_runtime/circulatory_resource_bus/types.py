"""CIR types — Circulatory Resource Bus is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise CirValidationError("cir.validation.secret", "secrets forbidden in CIR records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise CirValidationError("cir.validation.authority_created", f"{label} must not set authority_created=true")


def classify_cir_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
...
class CirculatoryRecord:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="CirculatoryRecord")
        if self.permission_granted:
            raise CirValidationError("cir.validation.permission_granted", "CIR must not grant permission")
        if not self.record_id.startswith("cir:"):
            raise CirValidationError("cir.validation.record_id", "record_id must start with cir:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": CIR_SCHEMA_VERSION,
            ...
            "circulatory_is_advisory_only": True,
            "proposal_only": True,
        }
        if include_hash:
...
class ResourceFlowSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="ResourceFlowSignal")
        if self.permission_granted:
            raise CirValidationError("cir.validation.permission_granted", "signal must not grant permission")
...
class CirculatoryReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="CirculatoryReceipt")
        for flag, label in (
            ...
            if flag:
                raise CirValidationError(f"cir.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise CirValidationError("cir.validation.negative_proof", f"{key} must be false")


def cir_record_from_fixture(data: dict[str, Any]) -> CirculatoryRecord:
    return CirculatoryRecord(
        record_id=str(data.get("record_id", "cir:fixture")),
