"""HRT types — Heartbeat & Liveness Transport is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise HrtValidationError("hrt.validation.secret", "secrets forbidden in HRT records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise HrtValidationError("hrt.validation.authority_created", f"{label} must not set authority_created=true")


def classify_hrt_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
...
class HeartbeatRecord:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="HeartbeatRecord")
        if self.permission_granted:
            raise HrtValidationError("hrt.validation.permission_granted", "HRT must not grant permission")
        if not self.record_id.startswith("hrt:"):
            raise HrtValidationError("hrt.validation.record_id", "record_id must start with hrt:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": HRT_SCHEMA_VERSION,
            ...
            "liveness_is_advisory_only": True,
            "proposal_only": True,
        }
        if include_hash:
...
class LivenessSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="LivenessSignal")
        if self.permission_granted:
            raise HrtValidationError("hrt.validation.permission_granted", "signal must not grant permission")
...
class HeartbeatReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="HeartbeatReceipt")
        for flag, label in (
            ...
            if flag:
                raise HrtValidationError(f"hrt.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise HrtValidationError("hrt.validation.negative_proof", f"{key} must be false")


def hrt_record_from_fixture(data: dict[str, Any]) -> HeartbeatRecord:
    return HeartbeatRecord(
        record_id=str(data.get("record_id", "hrt:fixture")),
