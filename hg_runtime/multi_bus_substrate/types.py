"""MBS types — multi-bus substrate is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise MBSValidationError("mbs.validation.secret", "secrets forbidden in MBS records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise MBSValidationError("mbs.validation.authority_created", f"{label} must not set authority_created=true")


def classify_mbs_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
    if "bus lane grants execution" in lower:
        return "bus_as_permission"
    if "bypass lane eligibility rules" in lower:
        return "lane_bypass"
    if "ignore bus saturation warnings" in lower:
        return "saturation_ignore"
    if "mint gpp permit from bus traffic" in lower:
...
class BusMessageRecord:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="BusMessageRecord")
        if self.permission_granted:
            raise MBSValidationError("mbs.validation.permission_granted", "MBS must not grant permission")
        if not self.record_id.startswith("mbs:"):
            raise MBSValidationError("mbs.validation.record_id", "record_id must start with mbs:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": MBS_SCHEMA_VERSION,
            ...
            "bus_is_advisory_only": True,
            "proposal_only": True,
            "bus_lane": self.bus_lane,
        }
        if include_hash:
...
class BusPressureSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="BusPressureSignal")
        if self.permission_granted:
            raise MBSValidationError("mbs.validation.permission_granted", "signal must not grant permission")
...
class BusReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="BusReceipt")
        for flag, label in (
            ...
            if flag:
                raise MBSValidationError(f"mbs.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in (
            ...
            if payload.get(key) is not False:
                raise MBSValidationError("mbs.validation.negative_proof", f"{key} must be false")


def mbs_record_from_fixture(data: dict[str, Any]) -> BusMessageRecord:
    kwargs: dict[str, Any] = {
        "record_id": str(data.get("record_id", "mbs:fixture")),
    ...
    kwargs["bus_lane"] = str(data.get("bus_lane", "proof"))
    return BusMessageRecord(**kwargs)
