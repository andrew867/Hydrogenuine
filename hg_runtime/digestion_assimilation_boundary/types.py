"""DAB types — digestion assimilation boundary is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise DabValidationError("dab.validation.secret", "secrets forbidden in DAB records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise DabValidationError("dab.validation.authority_created", f"{label} must not set authority_created=true")


def classify_dab_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
    if "write live memory from digestion" in lower:
        return "memory_write"
    if "install tool from digestion proposal" in lower:
        return "tool_install"
    if "digestion grants execution authority" in lower:
        return "execution_authority"
    if "mint gpp from digestion packet" in lower:
...
class DigestionRequest:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="DigestionRequest")
        if self.permission_granted:
            raise DabValidationError("dab.validation.permission_granted", "DAB must not grant permission")
        if not self.record_id.startswith("dab:"):
            raise DabValidationError("dab.validation.record_id", "record_id must start with dab:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": DAB_SCHEMA_VERSION,
            ...
            "digestion_is_advisory_only": True,
            "proposal_only": True,
        }
        if include_hash:
...
class DigestionSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="DigestionSignal")
        if self.permission_granted:
            raise DabValidationError("dab.validation.permission_granted", "signal must not grant permission")
...
class DigestionReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="DigestionReceipt")
        for flag, label in (
            ...
            if flag:
                raise DabValidationError(f"dab.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise DabValidationError("dab.validation.negative_proof", f"{key} must be false")


def dab_record_from_fixture(data: dict[str, Any]) -> DigestionRequest:
    return DigestionRequest(
        record_id=str(data.get("record_id", "dab:fixture")),
