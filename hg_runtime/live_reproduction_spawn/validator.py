"""RIB-SPAWN-LIVE validator — IAM/TIM/GPP/UEAK checks; child inherits no authority."""
...
def refuse_rib_spawn_as_authority(*, treat_as_authority: bool) -> None:
    if treat_as_authority:
        raise ValueError(REFUSED_SPAWN_AS_AUTHORITY)
...
def validate_spawn_request(
    ...
    *,
    ...
    """Validate spawn request; spawn plans require IAM + TIM + expiry + distinct child identity."""
    ...
    if inherit_parent_permit:
        return _base_result(
            status="contained",
            reason_code=REFUSED_INHERITED_AUTHORITY,
            request_id=request.request_id,
            evidence_admissible=False,
        )
    ...
    if not child_identity_distinct(request.parent_iam_ref, request.child_iam_ref):
        return _base_result(
            status="refused",
            reason_code=REFUSED_CHILD_IDENTITY_COLLISION,
            request_id=request.request_id,
            evidence_admissible=False,
        )
...
__all__ = ["refuse_rib_spawn_as_authority", "validate_spawn_request"]
