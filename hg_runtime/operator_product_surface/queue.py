"""EXCITON fake operator action queue — slice 3, no live dispatch."""
...
from hg_core.exciton_cluster.errors import EXCITON_FAKE_QUEUE_ENQUEUED
...
from hg_runtime.operator_product_surface.router import route_operator_bundle
from hg_runtime.operator_product_surface.types import FIXTURE_CLOCK


@dataclass
class FakeOperatorActionQueue:
    queue_id: str = "ops-fake-queue"
    entries: list[dict[str, Any]] = field(default_factory=list)

    def enqueue_bundle(self, bundle: dict[str, Any], *, observed_at: str = FIXTURE_CLOCK) -> dict[str, object]:
        routed = route_operator_bundle(bundle, observed_at=observed_at)
        entry = {
...
def enqueue_fixture_queue(*, observed_at: str = FIXTURE_CLOCK) -> dict[str, object]:
    ...
    return {
        **advisory_only_marker(),
        "status": "enqueued",
        "reason_code": EXCITON_FAKE_QUEUE_ENQUEUED,
        "fake_queue_only": True,
...
__all__ = ["FakeOperatorActionQueue", "enqueue_fixture_queue"]
