"""Append-only knowledge-acquisition record log with deterministic replay."""
...
from hg_runtime.knowledge_acquisition.schemas import preempt_if_needed

GENESIS_HASH = "sha256:phase30_genesis"
...
class KnowledgeAcquisitionLog:
    """Append-only JSONL log of acquisition records, hash-chained for replay."""
    ...
    def append(self, schema: str, payload: Mapping[str, Any], *, created_at: str | None = None) -> AcquisitionRecord:
        previous = self._head_hash or GENESIS_HASH
        enriched = dict(payload)
        payload_hash = canonical_hash(enriched)
        base = {
            "record_id": "ka-" + canonical_hash({"schema": schema, "payload_hash": payload_hash, "previous": previous}).removeprefix("sha256:")[:20],
            "schema": schema,
    ...
    def iter_records(self) -> list[AcquisitionRecord]:
    ...
    def replay(self, *, control: OperationControl | None = None) -> AcquisitionReplayResult:
        ...
        for record in self.iter_records():
            ...
            head = previous
            count += 1
        return AcquisitionReplayResult(ok=not errors, records=count, chain_root=head, errors=errors)
