"""ALC types — Agent Lifecycle Controller is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise AlcValidationError("alc.validation.secret", "secrets forbidden in ALC records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise AlcValidationError("alc.validation.authority_created", f"{label} must not set authority_created=true")


def classify_alc_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
...
class LifecycleRecord:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="LifecycleRecord")
        if self.permission_granted:
            raise AlcValidationError("alc.validation.permission_granted", "ALC must not grant permission")
        if not self.record_id.startswith("alc:"):
            raise AlcValidationError("alc.validation.record_id", "record_id must start with alc:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": ALC_SCHEMA_VERSION,
            ...
            "lifecycle_is_advisory_only": True,
            "proposal_only": True,
        }
        if include_hash:
...
class LifecycleSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="LifecycleSignal")
        if self.permission_granted:
            raise AlcValidationError("alc.validation.permission_granted", "signal must not grant permission")
...
class LifecycleReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="LifecycleReceipt")
        for flag, label in (
            ...
            if flag:
                raise AlcValidationError(f"alc.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise AlcValidationError("alc.validation.negative_proof", f"{key} must be false")


def alc_record_from_fixture(data: dict[str, Any]) -> LifecycleRecord:
    return LifecycleRecord(
        record_id=str(data.get("record_id", "alc:fixture")),
