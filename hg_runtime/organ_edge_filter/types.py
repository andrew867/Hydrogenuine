"""OEF types — organ edge filter is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise OEFValidationError("oef.validation.secret", "secrets forbidden in OEF records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise OEFValidationError("oef.validation.authority_created", f"{label} must not set authority_created=true")


def classify_oef_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
    if "edge filter pass grants execution" in lower:
        return "filter_as_permission"
    if "authority-bearing ingress allowed" in lower:
        return "authority_bearing"
    if "rate limit bypass at organ edge" in lower:
        return "rate_exceeded"
    if "mint gpp permit from edge filter" in lower:
...
class EdgeFilterRequest:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="EdgeFilterRequest")
        if self.permission_granted:
            raise OEFValidationError("oef.validation.permission_granted", "OEF must not grant permission")
        if not self.record_id.startswith("oef:"):
            raise OEFValidationError("oef.validation.record_id", "record_id must start with oef:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": OEF_SCHEMA_VERSION,
...
class EdgeFilterSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="EdgeFilterSignal")
        if self.permission_granted:
            raise OEFValidationError("oef.validation.permission_granted", "signal must not grant permission")
...
class EdgeFilterReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="EdgeFilterReceipt")
        for flag, label in (
            ...
            if flag:
                raise OEFValidationError(f"oef.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in (
            ...
            if payload.get(key) is not False:
                raise OEFValidationError("oef.validation.negative_proof", f"{key} must be false")


def oef_record_from_fixture(data: dict[str, Any]) -> EdgeFilterRequest:
    kwargs: dict[str, Any] = {
        "record_id": str(data.get("record_id", "oef:fixture")),
    ...
    return EdgeFilterRequest(**kwargs)
