"""Append-only local-provider-smoke record log with deterministic replay.

Probe, smoke, load, and unload records are hash-chained so the smoke is reproducible
and any tampering fails replay. Replay divergence is a failure, not a warning.
"""
...
from hg_runtime.local_provider_smoke.schemas import preempt_if_needed

GENESIS_HASH = "sha256:phase335_genesis"
...
class LocalProviderSmokeLog:
    """Append-only JSONL log of provider-smoke records, hash-chained for replay."""
    ...
    def append(self, schema: str, payload: Mapping[str, Any], *, created_at: str | None = None, control=None) -> SmokeRecord:
        preempt_if_needed(control)
        previous = self._head_hash or GENESIS_HASH
        enriched = dict(payload)
        payload_hash = canonical_hash(enriched)
        base = {
            "record_id": "lps-" + canonical_hash({"schema": schema, "payload_hash": payload_hash, "previous": previous}).removeprefix("sha256:")[:20],
            "schema": schema,
    ...
    def iter_records(self) -> list[SmokeRecord]:
    ...
    def replay(self, *, control: OperationControl | None = None) -> SmokeReplayResult:
        ...
        for record in self.iter_records():
            ...
            head = previous
            count += 1
        return SmokeReplayResult(ok=not errors, records=count, chain_root=head, errors=errors)
