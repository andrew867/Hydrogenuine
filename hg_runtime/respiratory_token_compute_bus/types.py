"""RSP types — Respiratory Token/Compute Bus is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise RspValidationError("rsp.validation.secret", "secrets forbidden in RSP records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise RspValidationError("rsp.validation.authority_created", f"{label} must not set authority_created=true")


def classify_rsp_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
...
class RespiratoryRecord:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="RespiratoryRecord")
        if self.permission_granted:
            raise RspValidationError("rsp.validation.permission_granted", "RSP must not grant permission")
        if not self.record_id.startswith("rsp:"):
            raise RspValidationError("rsp.validation.record_id", "record_id must start with rsp:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": RSP_SCHEMA_VERSION,
            ...
            "respiratory_is_advisory_only": True,
            "proposal_only": True,
        }
        if include_hash:
...
class TokenComputeSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="TokenComputeSignal")
        if self.permission_granted:
            raise RspValidationError("rsp.validation.permission_granted", "signal must not grant permission")
...
class RespiratoryReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="RespiratoryReceipt")
        for flag, label in (
            ...
            if flag:
                raise RspValidationError(f"rsp.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise RspValidationError("rsp.validation.negative_proof", f"{key} must be false")


def rsp_record_from_fixture(data: dict[str, Any]) -> RespiratoryRecord:
    return RespiratoryRecord(
        record_id=str(data.get("record_id", "rsp:fixture")),
