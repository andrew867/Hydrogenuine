"""BRB types — breathing regulation boundary is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise BrbValidationError("brb.validation.secret", "secrets forbidden in BRB records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise BrbValidationError("brb.validation.authority_created", f"{label} must not set authority_created=true")


def classify_brb_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
    ...
    if "cadence bypasses resource governance" in lower:
        return "resource_bypass"
    if "mint gpp permit from breath pressure" in lower:
...
class BreathCycleRecord:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="BreathCycleRecord")
        if self.permission_granted:
            raise BrbValidationError("brb.validation.permission_granted", "BRB must not grant permission")
        if not self.record_id.startswith("brb:"):
            raise BrbValidationError("brb.validation.record_id", "record_id must start with brb:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": BRB_SCHEMA_VERSION,
            ...
            "breathing_is_advisory_only": True,
            "proposal_only": True,
        }
        if include_hash:
...
class BreathPressureSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="BreathPressureSignal")
        if self.permission_granted:
            raise BrbValidationError("brb.validation.permission_granted", "signal must not grant permission")
...
class BreathReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="BreathReceipt")
        for flag, label in (
            ...
            if flag:
                raise BrbValidationError(f"brb.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise BrbValidationError("brb.validation.negative_proof", f"{key} must be false")


def brb_record_from_fixture(data: dict[str, Any]) -> BreathCycleRecord:
    return BreathCycleRecord(
        record_id=str(data.get("record_id", "brb:fixture")),
