"""Append-only generalization-evaluation record log with deterministic replay."""
...
from hg_runtime.generalization_eval.schemas import preempt_if_needed

GENESIS_HASH = "sha256:phase31_genesis"
...
class GeneralizationEvalLog:
    """Append-only JSONL log of evaluation records, hash-chained for replay."""
    ...
    def append(self, schema: str, payload: Mapping[str, Any], *, created_at: str | None = None) -> EvalRecord:
        previous = self._head_hash or GENESIS_HASH
        enriched = dict(payload)
        payload_hash = canonical_hash(enriched)
        base = {
            "record_id": "ge-" + canonical_hash({"schema": schema, "payload_hash": payload_hash, "previous": previous}).removeprefix("sha256:")[:20],
            "schema": schema,
    ...
    def iter_records(self) -> list[EvalRecord]:
    ...
    def replay(self, *, control: OperationControl | None = None) -> EvalReplayResult:
        ...
        for record in self.iter_records():
            ...
            head = previous
            count += 1
        return EvalReplayResult(ok=not errors, records=count, chain_root=head, errors=errors)
