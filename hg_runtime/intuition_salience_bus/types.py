"""ISB types — Intuition/Salience Bus is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise IsbValidationError("isb.validation.secret", "secrets forbidden in ISB records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise IsbValidationError("isb.validation.authority_created", f"{label} must not set authority_created=true")


def classify_isb_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
...
class SalienceRecord:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="SalienceRecord")
        if self.permission_granted:
            raise IsbValidationError("isb.validation.permission_granted", "ISB must not grant permission")
        if not self.record_id.startswith("isb:"):
            raise IsbValidationError("isb.validation.record_id", "record_id must start with isb:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": ISB_SCHEMA_VERSION,
            ...
            "salience_is_advisory_only": True,
            "proposal_only": True,
        }
        if include_hash:
...
class SalienceSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="SalienceSignal")
        if self.permission_granted:
            raise IsbValidationError("isb.validation.permission_granted", "signal must not grant permission")
...
class SalienceBusReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="SalienceBusReceipt")
        for flag, label in (
            ...
            if flag:
                raise IsbValidationError(f"isb.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise IsbValidationError("isb.validation.negative_proof", f"{key} must be false")


def isb_record_from_fixture(data: dict[str, Any]) -> SalienceRecord:
    return SalienceRecord(
        record_id=str(data.get("record_id", "isb:fixture")),
