"""AIS types — autonomic inference substrate is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise AISValidationError("ais.validation.secret", "secrets forbidden in AIS records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise AISValidationError("ais.validation.authority_created", f"{label} must not set authority_created=true")


def classify_ais_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
    if "inference output grants execution" in lower:
        return "inference_as_permission"
    if "invoke vllm live model" in lower:
        return "live_model_invoke"
    if "grant token budget from inference" in lower:
        return "budget_grant"
    if "mint gpp permit from inference" in lower:
...
class InferenceRequest:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="InferenceRequest")
        if self.permission_granted:
            raise AISValidationError("ais.validation.permission_granted", "AIS must not grant permission")
        if not self.record_id.startswith("ais:"):
            raise AISValidationError("ais.validation.record_id", "record_id must start with ais:")
...
class InferencePressureSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="InferencePressureSignal")
        if self.permission_granted:
            raise AISValidationError("ais.validation.permission_granted", "signal must not grant permission")
...
class InferenceReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="InferenceReceipt")
        for flag, label in (
            ...
            if flag:
                raise AISValidationError(f"ais.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in (
            ...
            if payload.get(key) is not False:
                raise AISValidationError("ais.validation.negative_proof", f"{key} must be false")


def ais_record_from_fixture(data: dict[str, Any]) -> InferenceRequest:
    kwargs: dict[str, Any] = {
        "record_id": str(data.get("record_id", "ais:fixture")),
