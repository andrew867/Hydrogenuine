"""RDB types — Reproduction/Delegation Bus is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise RdbValidationError("rdb.validation.secret", "secrets forbidden in RDB records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise RdbValidationError("rdb.validation.authority_created", f"{label} must not set authority_created=true")


def classify_rdb_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
...
class DelegationRecord:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="DelegationRecord")
        if self.permission_granted:
            raise RdbValidationError("rdb.validation.permission_granted", "RDB must not grant permission")
        if not self.record_id.startswith("rdb:"):
            raise RdbValidationError("rdb.validation.record_id", "record_id must start with rdb:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": RDB_SCHEMA_VERSION,
            ...
            "delegation_is_advisory_only": True,
            "proposal_only": True,
        }
        if include_hash:
...
@dataclass(frozen=True)
class DelegationPressureSignal:
    signal_id: str
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="DelegationPressureSignal")
        if self.permission_granted:
            raise RdbValidationError("rdb.validation.permission_granted", "signal must not grant permission")
...
class DelegationBusReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="DelegationBusReceipt")
        for flag, label in (
            ...
            if flag:
                raise RdbValidationError(f"rdb.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise RdbValidationError("rdb.validation.negative_proof", f"{key} must be false")


def rdb_record_from_fixture(data: dict[str, Any]) -> DelegationRecord:
    return DelegationRecord(
        record_id=str(data.get("record_id", "rdb:fixture")),
