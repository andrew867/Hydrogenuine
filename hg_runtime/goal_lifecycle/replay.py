"""Append-only goal-lifecycle record log with deterministic replay."""
...
from hg_runtime.goal_lifecycle.schemas import preempt_if_needed

GENESIS_HASH = "sha256:phase32_genesis"
...
class GoalLifecycleLog:
    """Append-only JSONL log of goal-lifecycle records, hash-chained for replay."""
    ...
    def append(self, schema: str, payload: Mapping[str, Any], *, created_at: str | None = None) -> GoalRecord:
        previous = self._head_hash or GENESIS_HASH
        enriched = dict(payload)
        payload_hash = canonical_hash(enriched)
        base = {
            "record_id": "gl-" + canonical_hash({"schema": schema, "payload_hash": payload_hash, "previous": previous}).removeprefix("sha256:")[:20],
            "schema": schema,
    ...
    def iter_records(self) -> list[GoalRecord]:
    ...
    def replay(self, *, control: OperationControl | None = None) -> GoalReplayResult:
        ...
        for record in self.iter_records():
            ...
            head = previous
            count += 1
        return GoalReplayResult(ok=not errors, records=count, chain_root=head, errors=errors)
