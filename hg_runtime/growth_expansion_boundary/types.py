"""GXB types — growth expansion boundary is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise GxbValidationError("gxb.validation.secret", "secrets forbidden in GXB records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise GxbValidationError("gxb.validation.authority_created", f"{label} must not set authority_created=true")


def classify_gxb_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
    if "growth grants permission to expand" in lower:
        return "growth_as_grant"
    if "spawn agents from growth pressure" in lower:
        return "agent_spawn"
    if "grant tools from growth request" in lower:
        return "tool_grant"
    if "allocate budget from growth pressure" in lower:
        return "budget_grant"
    if "scarcity self-authorize growth" in lower:
...
class GrowthRequest:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="GrowthRequest")
        if self.permission_granted:
            raise GxbValidationError("gxb.validation.permission_granted", "GXB must not grant permission")
        if not self.record_id.startswith("gxb:"):
            raise GxbValidationError("gxb.validation.record_id", "record_id must start with gxb:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": GXB_SCHEMA_VERSION,
...
class GrowthPressureSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="GrowthPressureSignal")
        if self.permission_granted:
            raise GxbValidationError("gxb.validation.permission_granted", "signal must not grant permission")
...
class GrowthReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="GrowthReceipt")
        for flag, label in (
            ...
            if flag:
                raise GxbValidationError(f"gxb.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise GxbValidationError("gxb.validation.negative_proof", f"{key} must be false")


def gxb_record_from_fixture(data: dict[str, Any]) -> GrowthRequest:
    return GrowthRequest(
        record_id=str(data.get("record_id", "gxb:fixture")),
