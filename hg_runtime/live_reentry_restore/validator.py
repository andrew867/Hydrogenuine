"""REB-RESTORE-LIVE validator — IAM/TIM/checkpoint/continuity checks."""
...
from hg_runtime.live_reentry_restore.types import (
    ...
    is_identity_overclaim,
    is_revoked_permit,
    is_stale_memory_claim,
...
def refuse_reb_as_authority(*, treat_as_authority: bool) -> None:
    if treat_as_authority:
        raise ValueError(REFUSED_REB_AS_AUTHORITY)
...
def validate_checkpoint_restore_request(
    ...
    *,
    ...
    if contains_leak({"checkpoint_digest": request.checkpoint_digest, "operator_ref": request.operator_ref}):
    ...
    if is_revoked_permit(request.gpp_permit_ref) or is_revoked_permit(request.revoked_permit_ref):
        return _base_result(status="refused", reason_code=REFUSED_REVOKED_PERMIT, request_id=request.request_id, evidence_admissible=False)

    if is_stale_memory_claim(request.stale_memory_ref):
        return _base_result(status="refused", reason_code=REFUSED_STALE_MEMORY_CLAIM, request_id=request.request_id, evidence_admissible=False)

    if is_identity_overclaim(request.continuity_claim_ref):
        return _base_result(status="refused", reason_code=REFUSED_IDENTITY_OVERCLAIM, request_id=request.request_id, evidence_admissible=False)
    ...
    return _base_result(
        status="recorded", reason_code=REB_RESTORE_CANDIDATE_BOUND, request_id=request.request_id,
...
__all__ = ["refuse_reb_as_authority", "validate_checkpoint_restore_request"]
