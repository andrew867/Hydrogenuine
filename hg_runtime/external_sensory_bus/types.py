"""ESB types — External Sensory Bus is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise EsbValidationError("esb.validation.secret", "secrets forbidden in ESB records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise EsbValidationError("esb.validation.authority_created", f"{label} must not set authority_created=true")


def classify_esb_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
...
class SensoryCueRecord:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="SensoryCueRecord")
        if self.permission_granted:
            raise EsbValidationError("esb.validation.permission_granted", "ESB must not grant permission")
        if not self.record_id.startswith("esb:"):
            raise EsbValidationError("esb.validation.record_id", "record_id must start with esb:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": ESB_SCHEMA_VERSION,
...
class SensoryPressureSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="SensoryPressureSignal")
        if self.permission_granted:
            raise EsbValidationError("esb.validation.permission_granted", "signal must not grant permission")
...
class SensoryBusReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="SensoryBusReceipt")
        for flag, label in (
            ...
            if flag:
                raise EsbValidationError(f"esb.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise EsbValidationError("esb.validation.negative_proof", f"{key} must be false")


def esb_record_from_fixture(data: dict[str, Any]) -> SensoryCueRecord:
    return SensoryCueRecord(
        record_id=str(data.get("record_id", "esb:fixture")),
