"""WDB types — waste disposal boundary is not authority."""
...
def _validate_no_secrets(*values: str) -> None:
    for value in values:
        ...
        if "password=" in lower or "api_key=" in lower or "token=secret" in lower:
            raise WdbValidationError("wdb.validation.secret", "secrets forbidden in WDB records")


def _reject_authority_created(authority_created: bool, *, label: str) -> None:
    if authority_created:
        raise WdbValidationError("wdb.validation.authority_created", f"{label} must not set authority_created=true")


def classify_wdb_claim_risk(notes: str) -> str | None:
    lower = notes.lower()
    if "waste deletes records without authority" in lower:
        return "waste_as_deletion"
    if "delete memory from waste proposal" in lower:
        return "memory_deletion"
    if "erase audit trail from waste disposal" in lower:
        return "audit_erasure"
    if "delete proof bundle from waste" in lower:
        return "proof_deletion"
    if "tombstone grants deletion authority" in lower:
...
class WasteCandidate:
    ...
    def __post_init__(self) -> None:
        ...
        _reject_authority_created(self.authority_created, label="WasteCandidate")
        if self.permission_granted:
            raise WdbValidationError("wdb.validation.permission_granted", "WDB must not grant permission")
        if not self.record_id.startswith("wdb:"):
            raise WdbValidationError("wdb.validation.record_id", "record_id must start with wdb:")
    ...
    def to_payload(self, *, include_hash: bool = True) -> dict[str, Any]:
        payload: dict[str, Any] = {
            "schema_version": WDB_SCHEMA_VERSION,
...
class WasteSignal:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="WasteSignal")
        if self.permission_granted:
            raise WdbValidationError("wdb.validation.permission_granted", "signal must not grant permission")
...
class WasteReceipt:
    ...
    def __post_init__(self) -> None:
        _reject_authority_created(self.authority_created, label="WasteReceipt")
        for flag, label in (
            ...
            if flag:
                raise WdbValidationError(f"wdb.validation.{label}", f"{label} must remain false")
    ...
    def validate_negative_proofs(payload: dict[str, Any]) -> None:
        for key in ("authority_created", "permission_granted", "permit_minted", "execution_admitted", "memory_history_mutated", "deletion_performed", "tool_removed", "agent_spawned", "oea_ter_called"):
            if payload.get(key) is not False:
                raise WdbValidationError("wdb.validation.negative_proof", f"{key} must be false")


def wdb_record_from_fixture(data: dict[str, Any]) -> WasteCandidate:
    return WasteCandidate(
        record_id=str(data.get("record_id", "wdb:fixture")),
