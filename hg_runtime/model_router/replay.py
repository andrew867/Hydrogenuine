"""Append-only model-router record log with deterministic replay.

Routing and residency records (including hot-reload records) are hash-chained so a
hot reload preserves the audit chain and any tampering fails replay.
"""
...
from hg_runtime.model_router.schemas import preempt_if_needed

GENESIS_HASH = "sha256:phase33_genesis"
...
class ModelRouterLog:
    """Append-only JSONL log of router records, hash-chained for replay."""
    ...
    def append(self, schema: str, payload: Mapping[str, Any], *, created_at: str | None = None) -> RouterRecord:
        previous = self._head_hash or GENESIS_HASH
        enriched = dict(payload)
        payload_hash = canonical_hash(enriched)
        base = {
            "record_id": "mr-" + canonical_hash({"schema": schema, "payload_hash": payload_hash, "previous": previous}).removeprefix("sha256:")[:20],
            "schema": schema,
    ...
    def iter_records(self) -> list[RouterRecord]:
    ...
    def replay(self, *, control: OperationControl | None = None) -> RouterReplayResult:
        ...
        for record in self.iter_records():
            ...
            head = previous
            count += 1
        return RouterReplayResult(ok=not errors, records=count, chain_root=head, errors=errors)
