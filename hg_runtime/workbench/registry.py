"""Tool registry and append-only workbench receipt log."""
...
from hg_runtime.workbench.schemas import WorkbenchError, preempt_if_needed, validate_tool_registry_entry

GENESIS_HASH = "sha256:phase29_genesis"
...
class ToolRegistry:
    ...
    def register(self, entry: Mapping[str, Any]) -> dict[str, Any]:
        expected = canonical_hash({key: value for key, value in entry.items() if key != "registry_hash"})
        if entry.get("registry_hash") != expected:
            raise WorkbenchError("tool_registry_hash_mismatch")
        data = validate_tool_registry_entry(entry)
        self._tools[data["tool_id"]] = data
        return data
    ...
    def get(self, tool_id: str) -> dict[str, Any]:
        try:
            ...
            raise WorkbenchError("unknown_tool_rejected") from exc
...
class WorkbenchReceiptLog:
    ...
    def append(self, schema: str, payload: Mapping[str, Any], *, created_at: str | None = None) -> WorkbenchRecord:
        previous = self._head_hash or GENESIS_HASH
        enriched = dict(payload)
        payload_hash = canonical_hash(enriched)
        base = {
            "record_id": "wb-" + canonical_hash({"schema": schema, "payload_hash": payload_hash, "previous": previous}).removeprefix("sha256:")[:20],
            "schema": schema,
    ...
    def iter_records(self) -> list[WorkbenchRecord]:
    ...
    def replay(self, *, control: OperationControl | None = None) -> WorkbenchReplayResult:
        ...
        for record in self.iter_records():
            ...
            head = previous
            count += 1
        return WorkbenchReplayResult(ok=not errors, records=count, chain_root=head, errors=errors)


__all__ = ["GENESIS_HASH", "ToolRegistry", "WorkbenchReceiptLog", "WorkbenchReplayResult", "WorkbenchRecord"]
