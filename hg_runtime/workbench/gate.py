"""Phase 29 gate and proof bundle validation."""
...
VERDICT_GREEN = "GREEN_AUTONOMOUS_AGENT_ZERO_PHASE_29_TOOL_MEDIATED_WORKBENCH"
...
def validate_phase29_proof_bundle(path: Path | None) -> tuple[bool, list[str]]:
    if path is None:
        return False, ["RED_PHASE29_PROOF_BUNDLE_MISSING"]
    required = ["gate_result.json", "manifest.json", "summary.json", "command_log.jsonl", "HEAD.txt"]
    failures: list[str] = []
    for name in required:
        if not (path / name).is_file():
            failures.append(f"RED_PHASE29_PROOF_BUNDLE_MISSING:{name}")
    ...
    if gate_path.is_file():
        try:
            ...
            if gate.get("proof_bundle") and Path(str(gate["proof_bundle"])).resolve() != path.resolve():
                failures.append("RED_PHASE29_PROOF_BUNDLE_PATH_MISMATCH")
        except json.JSONDecodeError:
            failures.append("RED_PHASE29_GATE_RESULT_INVALID_JSON")
    return not failures, failures


def evaluate_phase29_gate(
    ...
    *,
    ...
    dry_run_cannot_mutate_workspace: bool = False,
    live_tool_requires_permit: bool = False,
    unknown_tools_rejected: bool = False,
    forbidden_commands_rejected: bool = False,
    ...
    artifact_hashes_required: bool = False,
    ...
    if not phase26_green:
        failures.append("RED_PHASE29_PHASE26_GREEN_REQUIRED")
    if not phase28_green:
        failures.append("RED_PHASE29_PHASE28_GREEN_REQUIRED")
    proof_ok, proof_failures = validate_phase29_proof_bundle(proof_bundle)
    failures.extend(proof_failures)
    checks = {
        "tests_passed": tests_passed,
        "proof_bundle_valid": proof_ok,
        "report_exists": report_exists,
        "dry_run_cannot_mutate_workspace": dry_run_cannot_mutate_workspace,
        "live_tool_requires_permit": live_tool_requires_permit,
        "unknown_tools_rejected": unknown_tools_rejected,
        "forbidden_commands_rejected": forbidden_commands_rejected,
        ...
        "artifact_hashes_required": artifact_hashes_required,
    ...
    for key, ok in checks.items():
        if not ok:
            failures.append(f"RED_PHASE29_{key.upper()}_FAIL")
...
def main() -> int:
    report = WORKSPACE / "docs/reports/phases/AUTONOMOUS_AGENT_ZERO_PHASE_29_TOOL_MEDIATED_WORKBENCH_REPORT.md"
    proof_dir = WORKSPACE / "docs/proofs/autonomous_agent_zero/PHASE-29-TOOL-MEDIATED-WORKBENCH" / _stamp()
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "pytest", "tests/autonomous_agent/test_phase29_workbench.py", "-q"],
        cwd=WORKSPACE,
        capture_output=True,
        text=True,
        timeout=300,
    ...
    command_log = [
        ...
        {
            "command": "python -m pytest tests/autonomous_agent/test_phase29_workbench.py -q",
    ...
    (proof_dir / "manifest.json").write_text(json.dumps({"phase": 29, "head": head}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    (proof_dir / "gate_result.json").write_text(json.dumps({"proof_bundle": str(proof_dir)}, indent=2) + "\n", encoding="utf-8")
    preliminary = {"phase": 29, "head": head, "proof_bundle": str(proof_dir), "checks_passed": proc.returncode == 0 and phase26_ok and phase28_ok}
    (proof_dir / "summary.json").write_text(json.dumps(preliminary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ok = proc.returncode == 0
    result = evaluate_phase29_gate(
        WORKSPACE,
        ...
        dry_run_cannot_mutate_workspace=ok,
        live_tool_requires_permit=ok,
        unknown_tools_rejected=ok,
        forbidden_commands_rejected=ok,
        credential_reads_rejected=ok,
        domain_packs_cannot_authorize_tools=ok,
        skills_cannot_authorize_tools=ok,
        memory_cannot_authorize_tools=ok,
        artifact_hashes_required=ok,
        receipts_required=ok,
    ...
    (proof_dir / "status.md").write_text(f"# Phase 29 Gate\n\nVerdict: `{result['verdict']}`\n", encoding="utf-8")
...
__all__ = ["VERDICT_GREEN", "evaluate_phase29_gate", "validate_phase29_proof_bundle"]
