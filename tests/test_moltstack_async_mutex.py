#!/usr/bin/env python3
"""
Tests for moltstack_api_client_async.py mutex integration - Phase 6
...
class TestAsyncMutexManager(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('moltstack.moltstack_api_client_async.posting_lock_with_content')
    @patch('moltstack.moltstack_api_client_async.asyncio.to_thread')
...
class TestMoltstackAsyncClientMutex(unittest.IsolatedAsyncioTestCase):
    """Test MoltstackAsyncClient with mutex integration."""
    
    def setUp(self):
        """Set up test client."""
        # Mock API key
        with patch('moltstack.moltstack_api_client_async.load_api_key', return_value="test_api_key"):
            self.client = MoltstackAsyncClient(api_key="test_api_key")
    
    @patch('moltstack.moltstack_api_client_async.httpx.AsyncClient')
    @patch('moltstack.moltstack_api_client_async.get_content_hash')
    @patch('moltstack.moltstack_api_client_async.get_posting_lock_path')
    @patch('moltstack.moltstack_api_client_async.AsyncMutexManager')
    async def test_post_request_with_mutex(self, mock_mutex_manager, mock_lock_path, mock_get_hash, mock_client_class):
        ...
        # Make POST request
        result = await self.client._request("POST", "/posts", json={"title": "Test", "content": "Test"})
...
class TestCreatePostMutex(unittest.IsolatedAsyncioTestCase):
    ...
    @patch('moltstack.moltstack_api_client_async.get_posting_lock_path')
    @patch('moltstack.moltstack_api_client_async.AsyncMutexManager')
    @patch('moltstack.moltstack_api_client_async.markdown_to_html')
    async def test_create_post_uses_mutex(self, mock_markdown, mock_mutex_manager, mock_lock_path, mock_get_hash, mock_client_class):
        """Test that create_post uses mutex through _request."""
        # Setup mocks
        mock_get_hash.return_value = "test_hash_123"
        mock_lock_path.return_value = Path("/tmp/test.lock")
        mock_markdown.return_value = "<p>Test content</p>"
        ...
        # Create post
        result = await self.client.create_post("Test Title", "Test content")
