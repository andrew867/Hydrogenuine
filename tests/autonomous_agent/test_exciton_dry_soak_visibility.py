"""EXCITON dry soak visibility tests."""
...
def test_dry_soak_panel_registered():
    assert "AgentZeroDrySoakMonitorPanel" in CONTRACT_BY_ID
    contract = CONTRACT_BY_ID["AgentZeroDrySoakMonitorPanel"]
    assert "publish_social" in contract.forbidden_controls


def test_panel_has_truth_freshness_when_no_run():
    panels = build_agent_zero_dry_soak_panels(CollectorContext(offline_fixture=False))
    assert len(panels) == 1
    panel = panels[0]
...
def test_stale_heartbeat_not_green():
    panels = build_agent_zero_dry_soak_panels(CollectorContext(offline_fixture=True))
    panel = panels[0]
