"""EXCITON dry autonomous loop visibility tests."""
...
def test_dry_autonomous_loop_panel_registered():
    assert "AgentZeroDryAutonomousLoopMonitorPanel" in CONTRACT_BY_ID
    contract = CONTRACT_BY_ID["AgentZeroDryAutonomousLoopMonitorPanel"]
    assert "publish_social" in contract.forbidden_controls


def test_panel_has_truth_freshness_when_no_run():
    panels = build_agent_zero_dry_autonomous_loop_panels(CollectorContext(offline_fixture=False))
    assert len(panels) == 1
    panel = panels[0]
    assert panel.fields.get("truth_state")
    assert panel.state == ExcitonPanelState.RED or panel.fields.get("freshness_status") is not None


def test_stale_heartbeat_not_green():
    panels = build_agent_zero_dry_autonomous_loop_panels(CollectorContext(offline_fixture=True))
    panel = panels[0]
