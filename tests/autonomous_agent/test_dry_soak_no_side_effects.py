"""Phase 12 no external side effects."""
...
def test_no_live_write_modules():
    pkg = WORKSPACE / "hg_runtime/dry_soak"
...
def test_no_pass_stubs():
    for py in (WORKSPACE / "hg_runtime/dry_soak").glob("*.py"):
        for line in py.read_text(encoding="utf-8").splitlines():
            assert line.strip() != "pass"


def test_no_publish_send_in_package():
    text = "\n".join(p.read_text(encoding="utf-8") for p in (WORKSPACE / "hg_runtime/dry_soak").glob("*.py"))
