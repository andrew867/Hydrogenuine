"""Batch L3-A lifecycle shell tests."""
...
def test_all_lifecycle_slices_green() -> None:
    result = run_lifecycle_batch_a_checks(WORKSPACE, slice="all")
    assert result["ok"], result.get("critical_failures", result)


def test_each_slice_green() -> None:
    for name in L3_A_SLICES:
        result = run_lifecycle_batch_a_checks(WORKSPACE, slice=name)
        assert result["ok"], (name, result)


def test_unsupported_slice_fails_closed() -> None:
    result = run_lifecycle_batch_a_checks(WORKSPACE, slice="pro")
