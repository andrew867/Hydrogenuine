"""Batch R2-A runtime context shell tests."""
...
def test_all_runtime_slices_green() -> None:
    result = run_runtime_context_batch_a_checks(WORKSPACE, slice="all")
    assert result["ok"], result.get("critical_failures", result)


def test_each_slice_green() -> None:
    for name in R2_A_SLICES:
        result = run_runtime_context_batch_a_checks(WORKSPACE, slice=name)
        assert result["ok"], (name, result)


def test_unsupported_slice_fails_closed() -> None:
    result = run_runtime_context_batch_a_checks(WORKSPACE, slice="fce")
