"""ISB Intuition/Salience Bus tests — fixture/static only."""

from __future__ import annotations

import pytest

from hg_core.isb_cluster.errors import REFUSED_ISB_AS_AUTHORITY, IsbValidationError
...
def _record(**overrides: object) -> SalienceRecord:
    base = {"record_id": "isb:req-test", "summary": "fixture test", "observed_at": FIXTURE_CLOCK, "classification": "stable"}
    base.update(overrides)
    return isb_record_from_fixture(base)
...
def test_record_schema_non_authority() -> None:
    ...
    assert payload["salience_is_advisory_only"] is True


def test_record_rejects_authority_created() -> None:
    with pytest.raises(IsbValidationError):
...
def test_signal_negative_proofs() -> None:
    signal = SalienceSignal(signal_id="isb:signal-test", pressure_score=0.2, observed_at=FIXTURE_CLOCK)
...
def test_receipt_negative_proofs() -> None:
    receipt = SalienceBusReceipt(receipt_id="isb:receipt-test", record_ref="isb:req-test", emitted_events=("ISB_RECORDED",))
    SalienceBusReceipt.validate_negative_proofs(receipt.to_payload())
    assert receipt.to_payload()["deletion_performed"] is False


def test_valid_bundle_recorded() -> None:
    bundle = next(b for b in load_isb_fixtures() if b["bundle_id"] == "isb-valid-salience")
    result = process_isb_bundle(bundle)
    assert result["status"] == "recorded"
    assert result["permission_granted"] is False


def test_fail_closed_path() -> None:
    bundle = next(b for b in load_isb_fixtures() if b["bundle_id"] == "isb-stale-input")
    result = process_isb_bundle(bundle)
    assert result["status"] in ("fail_closed", "contained")


def test_adversarial_contained() -> None:
    bundle = next(b for b in load_isb_fixtures() if b["bundle_id"] == "isb-token-grant")
    result = process_isb_bundle(bundle)
    assert result["status"] == "contained"
    assert result["permission_granted"] is False


def test_refuse_isb_as_authority() -> None:
    with pytest.raises(IsbValidationError) as exc:
        refuse_isb_as_authority(treat_as_authority=True)
    assert exc.value.code == REFUSED_ISB_AS_AUTHORITY


def test_deterministic_replay_hash() -> None:
    bundles = list(load_isb_fixtures()[:5])
...
def test_rtc_event_design() -> None:
    ok, failures = validate_isb_rtc_event_design(planned_isb_event_refs())
    assert ok, failures


def test_analyze_fixtures() -> None:
    analysis = analyze_isb_fixtures()
...
def test_context_grant_refused() -> None:
    bundle = next(b for b in load_isb_fixtures() if b["bundle_id"] == "isb-context-grant")
    result = process_isb_bundle(bundle)
    assert result["status"] == "contained"
    assert result["permission_granted"] is False
...
def test_execution_admission_refused() -> None:
    bundle = next(b for b in load_isb_fixtures() if b["bundle_id"] == "isb-execution-admission")
    result = process_isb_bundle(bundle)
    assert result["status"] == "contained"
    assert result["permission_granted"] is False
...
def test_resource_bypass_refused() -> None:
    bundle = next(b for b in load_isb_fixtures() if b["bundle_id"] == "isb-resource-bypass")
    result = process_isb_bundle(bundle)
    assert result["status"] == "contained"
    assert result["permission_granted"] is False
...
def test_authority_conversion_refused() -> None:
    bundle = next(b for b in load_isb_fixtures() if b["bundle_id"] == "isb-authority-conversion")
    result = process_isb_bundle(bundle)
    assert result["status"] == "contained"
    assert result["permission_granted"] is False
...
def test_classify_claim_risk_none_for_benign() -> None:
    assert classify_isb_claim_risk("observe stable signal") is None
