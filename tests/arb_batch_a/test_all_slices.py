...
def test_all_arb_a_slices_green() -> None:
    result = run_arb_batch_a_checks(WORKSPACE, slice="all")
    assert result["ok"], result.get("critical_failures", result)


def test_each_slice_green() -> None:
    for name in ARB_A_SLICES:
        result = run_arb_batch_a_checks(WORKSPACE, slice=name)
        assert result["ok"], (name, result)


def test_unsupported_slice_fails_closed() -> None:
    result = run_arb_batch_a_checks(WORKSPACE, slice="ipb")
    assert not result["ok"]
    assert "unsupported_slice" in result["critical_failures"]


def test_arb_import_fences_clean() -> None:
    ok, failures = check_arb_import_fences()
    assert ok, failures
