"""Batch P1-B policy safety shell tests."""
...
def test_all_policy_slices_green() -> None:
    result = run_policy_batch_b_checks(WORKSPACE, slice="all")
    assert result["ok"], result.get("critical_failures", result)


def test_each_slice_green() -> None:
    for name in P1_B_SLICES:
        result = run_policy_batch_b_checks(WORKSPACE, slice=name)
        assert result["ok"], (name, result)


def test_unsupported_slice_fails_closed() -> None:
    result = run_policy_batch_b_checks(WORKSPACE, slice="syn")
