"""ALC Agent Lifecycle Controller tests — fixture/static only."""

from __future__ import annotations

import pytest

from hg_core.alc_cluster.errors import REFUSED_ALC_AS_AUTHORITY, AlcValidationError
...
def _record(**overrides: object) -> LifecycleRecord:
    base = {"record_id": "alc:req-test", "summary": "fixture test", "observed_at": FIXTURE_CLOCK, "classification": "stable"}
    base.update(overrides)
    return alc_record_from_fixture(base)
...
def test_record_schema_non_authority() -> None:
    ...
    assert payload["lifecycle_is_advisory_only"] is True


def test_record_rejects_authority_created() -> None:
    with pytest.raises(AlcValidationError):
...
def test_signal_negative_proofs() -> None:
    signal = LifecycleSignal(signal_id="alc:signal-test", pressure_score=0.2, observed_at=FIXTURE_CLOCK)
...
def test_receipt_negative_proofs() -> None:
    receipt = LifecycleReceipt(receipt_id="alc:receipt-test", record_ref="alc:req-test", emitted_events=("ALC_RECORDED",))
    LifecycleReceipt.validate_negative_proofs(receipt.to_payload())
    assert receipt.to_payload()["deletion_performed"] is False


def test_valid_bundle_recorded() -> None:
    bundle = next(b for b in load_alc_fixtures() if b["bundle_id"] == "alc-valid-lifecycle")
    result = process_alc_bundle(bundle)
    assert result["status"] == "recorded"
    assert result["permission_granted"] is False


def test_fail_closed_path() -> None:
    bundle = next(b for b in load_alc_fixtures() if b["bundle_id"] == "alc-stale-input")
    result = process_alc_bundle(bundle)
    assert result["status"] in ("fail_closed", "contained")


def test_adversarial_contained() -> None:
    bundle = next(b for b in load_alc_fixtures() if b["bundle_id"] == "alc-token-grant")
    result = process_alc_bundle(bundle)
    assert result["status"] == "contained"
    assert result["permission_granted"] is False


def test_refuse_alc_as_authority() -> None:
    with pytest.raises(AlcValidationError) as exc:
        refuse_alc_as_authority(treat_as_authority=True)
    assert exc.value.code == REFUSED_ALC_AS_AUTHORITY


def test_deterministic_replay_hash() -> None:
    bundles = list(load_alc_fixtures()[:5])
...
def test_rtc_event_design() -> None:
    ok, failures = validate_alc_rtc_event_design(planned_alc_event_refs())
    assert ok, failures


def test_analyze_fixtures() -> None:
    analysis = analyze_alc_fixtures()
...
def test_context_grant_refused() -> None:
    bundle = next(b for b in load_alc_fixtures() if b["bundle_id"] == "alc-context-grant")
    result = process_alc_bundle(bundle)
    assert result["status"] == "contained"
    assert result["permission_granted"] is False
...
def test_execution_admission_refused() -> None:
    bundle = next(b for b in load_alc_fixtures() if b["bundle_id"] == "alc-execution-admission")
    result = process_alc_bundle(bundle)
    assert result["status"] == "contained"
    assert result["permission_granted"] is False
...
def test_resource_bypass_refused() -> None:
    bundle = next(b for b in load_alc_fixtures() if b["bundle_id"] == "alc-resource-bypass")
    result = process_alc_bundle(bundle)
    assert result["status"] == "contained"
    assert result["permission_granted"] is False
...
def test_authority_conversion_refused() -> None:
    bundle = next(b for b in load_alc_fixtures() if b["bundle_id"] == "alc-authority-conversion")
    result = process_alc_bundle(bundle)
    assert result["status"] == "contained"
    assert result["permission_granted"] is False
...
def test_classify_claim_risk_none_for_benign() -> None:
    assert classify_alc_claim_risk("observe stable signal") is None
