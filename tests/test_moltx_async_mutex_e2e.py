#!/usr/bin/env python3
"""
End-to-end tests for moltx_api_client_async.py mutex integration - Phase 5
...
async def test_e2e_same_content_blocking():
    ...
    with patch('skills.automation.moltx.moltx_api_client_async.load_api_key', return_value="test_api_key"):
        ...
        with patch('skills.automation.moltx.moltx_api_client_async.posting_lock_with_content') as mock_lock:
            ...
            async def make_request(req_id):
                try:
                    client = MoltxAsyncClient(api_key="test_api_key")
                    # Mock the httpx client
                    with patch('skills.automation.moltx.moltx_api_client_async.httpx.AsyncClient') as mock_client_class:
                        ...
                        result = await client._request("POST", "/posts", json={"content": "Same", "type": "post"})
        ...
        with patch('skills.automation.moltx.moltx_api_client_async.posting_lock_with_content') as mock_lock:
            ...
            async def make_request(content):
                ...
                with patch('skills.automation.moltx.moltx_api_client_async.httpx.AsyncClient') as mock_client_class:
                    ...
                    payload = {"content": content, "type": "post"}
            ...
            tasks = [
                make_request("Content A"),
                make_request("Content B"),
                make_request("Content C")
            ]
            await asyncio.gather(*tasks)
...
async def test_e2e_create_post_uses_mutex():
    ...
    with patch('skills.automation.moltx.moltx_api_client_async.load_api_key', return_value="test_api_key"):
        ...
        with patch('skills.automation.moltx.moltx_api_client_async.get_content_hash') as mock_get_hash:
            ...
            with patch('skills.automation.moltx.moltx_api_client_async.httpx.AsyncClient') as mock_client_class:
                ...
                result = await client.create_post("Test content")
...
async def main():
    print("=" * 60)
    print("End-to-End Tests for moltx_api_client_async.py Mutex (Async)")
    print("=" * 60 + "\n")
