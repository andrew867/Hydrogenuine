"""Batch D4-B developmental shell tests."""
...
def test_all_developmental_slices_green() -> None:
    result = run_developmental_batch_b_checks(WORKSPACE, slice="all")
    assert result["ok"], result.get("critical_failures", result)


def test_each_slice_green() -> None:
    for name in D4_B_SLICES:
        result = run_developmental_batch_b_checks(WORKSPACE, slice=name)
        assert result["ok"], (name, result)


def test_unsupported_slice_fails_closed() -> None:
    result = run_developmental_batch_b_checks(WORKSPACE, slice="els")
